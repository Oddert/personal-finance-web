...
export const intakeError = (error: any) => async (dispatch: AppDispatch) => {
    try {
        if (error instanceof AxiosError) {
            ...
            if (error.status === 401) {
                console.warn('[intakeError] condition 1.1');
                return;
            }
            // Condition 1.2: All other Axios errors.
            console.warn('[intakeError] condition 1.2');
            if (
                error?.response?.data &&
                ...
                console.warn('[intakeError] condition 1.2.1');
                dispatch(
                    writeError(
                        Object.assign(
                            {
                ...
                // Condition 1.2.2: The request is a generic Axios network error indicating failure to communicate with the server.
                console.warn('[intakeError] condition 1.2.2');
                dispatch(
                    writeError(
                        Object.assign(
                            {
                                ...
                                message:
                                    'The application was unable to reach the server, please check your network connection and try again.',
                                error: 'Application error',
                                stackTrace: '',
                            },
                            {
                ...
                // Condition 1.2.3: If the request failed to reach our backend or an unhandled error occurs, use Axios's response info.
                console.warn('[intakeError] condition 1.2.3');
                dispatch(
                    writeError(
                        Object.assign(
                            {
            ...
            console.warn('[intakeError] condition 2');
            dispatch(
                writeError(
                    Object.assign(
                        {
            ...
            console.warn('[intakeError] condition 3');
            dispatch(
                writeError({
                    title: 'Something went wrong',
                    message: 'An unexpected error was encountered.',
        ...
        console.warn('[intakeError] condition 4');
        console.error(err);
        dispatch(
            writeError({
